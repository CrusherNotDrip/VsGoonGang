...
class Paths
{
	...
	/*
	public static function scan(key:String, ?ext:String = '', ?recursive:Bool = true, ?returnType:ScanReturnType = ASSETS_PATH_FILE, ?returnExt:Bool = true):Array<String>
	{
		...
		{
			var assetKey:String = Path.join(['assets', key]);
		...
		#if MODS_ALLOWED
		var modScan:String->Void = null;

		modScan = (folder:String) ->
		{
			for (file in FileSystem.readDirectory(folder))
			{
				var modAssetKey:String = Path.join([folder, file]);

				if (FileSystem.isDirectory(file) && recursive)
				{
					modScan(modAssetKey);
				}
				else
				{
					if (!modAssetKey.endsWith(ext))
						continue;

					var modAssetToPush:String = switch (returnType)
					{
						case ASSETS_PATH_FILE:
							modAssetKey;

						case PATH_FILE:
							modAssetKey.split(folder)[1];
					...
					// Avoid duplicates especially if your removing all extentions.
					if (!foundAssets.contains(modAssetToPush))
						foundAssets.push(modAssetToPush);
				}
			}
		}
		...
		{
			modScan(mods(Path.join([mod, key])));
		}
...
// St olen from my own engine btw :3
enum ScanReturnType
{
	ASSETS_PATH_FILE;
	PATH_FILE;
